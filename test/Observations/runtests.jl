...
@testset "left-multiply utilities" begin
    ...
    # sqrt_cov
    # Don't test the SumOfCovariance type matrices (i.e. nosum)

    Γ = get_obs_noise_cov(observation_series_nosum)
    Γinv = get_obs_noise_cov_inv(observation_series_nosum)
    Xvec = randn(size(Γ, 1))
    X = randn(size(Γ, 1), 50)
    ...
    test1 = Γrt * Xvec
    test2 = Γrt * X
    @test norm(test1 - lmul_sqrt_obs_noise_cov(observation_series_nosum, Xvec)) < sqrt(1e-10)
    @test norm(test2 - lmul_sqrt_obs_noise_cov(observation_series_nosum, X)) < sqrt(1e-10)
    test1 = Γrt[g_idx, g_idx] * Xvec[g_idx]
    test2 = Γrt[g_idx, g_idx] * X[g_idx, :]
    out1 = zeros(size(test1))
    out2 = zeros(size(test2))
    lmul_sqrt_obs_noise_cov!(out1, observation_series_nosum, Xvec[g_idx], g_idx)
    lmul_sqrt_obs_noise_cov!(out2, observation_series_nosum, X[g_idx, :], g_idx)
    @test norm(test1 - out1) < sqrt(1e-10)
    @test norm(test2 - out2) < sqrt(1e-10)
    lmul_sqrt_obs_noise_cov!(out1, observation_series_nosum, Xvec, g_idx)
    lmul_sqrt_obs_noise_cov!(out2, observation_series_nosum, X, g_idx)
    @test norm(test1 - out1) < sqrt(1e-10)
    @test norm(test2 - out2) < sqrt(1e-10)
    ...
    test1 = Γrt_inv * Xvec
    test2 = Γrt_inv * X
    @test norm(test1 - lmul_sqrt_obs_noise_cov_inv(observation_series_nosum, Xvec)) < sqrt(1e-10)
    @test norm(test2 - lmul_sqrt_obs_noise_cov_inv(observation_series_nosum, X)) < sqrt(1e-10)
    test1 = Γrt_inv[g_idx, g_idx] * Xvec[g_idx]
    test2 = Γrt_inv[g_idx, g_idx] * X[g_idx, :]
    out1 = zeros(size(test1))
    out2 = zeros(size(test2))
    # In code this is applied to pre-trimmed "X"s or not
    lmul_sqrt_obs_noise_cov_inv!(out1, observation_series_nosum, Xvec[g_idx], g_idx)
    lmul_sqrt_obs_noise_cov_inv!(out2, observation_series_nosum, X[g_idx, :], g_idx)
    @test norm(test1 - out1) < sqrt(1e-10)
    @test norm(test2 - out2) < sqrt(1e-10)
    lmul_sqrt_obs_noise_cov_inv!(out1, observation_series_nosum, Xvec, g_idx)
    lmul_sqrt_obs_noise_cov_inv!(out2, observation_series_nosum, X, g_idx)
    @test norm(test1 - out1) < sqrt(1e-10)
    @test norm(test2 - out2) < sqrt(1e-10)
