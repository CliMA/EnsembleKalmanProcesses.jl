...
@testset "ParameterDistributions" begin
    ...
    @testset "statistics functions: explict RNG" begin
        ...
        rng1_new = copy(rng1)
        @test sample(copy(rng1), u3, 3) == cat([reshape(rand(rng1_new, test_d3a, 3),:,3), rand(rng1_new, test_d3b, 3)]...,dims=1)    
        ...
        rng2_new = copy(rng2)
        @test sample(copy(rng2), u3, 3) == cat([reshape(rand(rng2_new, test_d3a, 3),:,3),rand(rng2_new, test_d3b, 3)]...,dims=1)    
