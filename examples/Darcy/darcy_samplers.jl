...
corr_length = 0.25
dofs = 5
@info "Effective problem dimension: $dofs"
...
case_config = Dict(
    "inversion" =>  Dict(
        "process" => Inversion(),
        #"scheduler" => DefaultScheduler(1e-8),
        "scheduler" => DataMisfitController(terminate_at=1),
        "N_iterations" => 20,# Int(ceil(10.0/fixed_step))
    ),
    "sampler" =>  Dict(
    ...
    "nonrev_sampler" =>  Dict(
        "process" => NonreversibleSampler(prior, prefactor=3),
#        "scheduler" => DefaultScheduler(5e-8),
        "scheduler" => EKSStableScheduler(),
        "N_iterations" => 200,
    ),
)
...
u_dts = []
loss_stored = []
for config_key in keys(case_config)
    config = case_config[config_key]
    case = config_key
    ...
    # plot the decay of the mean and of the standard deviation
    ...
    push!(u_storeds, get_u(ekiobj, return_array = true))
    push!(g_storeds, get_g(ekiobj, return_array = true))
    push!(u_dts, get_Δt(ekiobj))
    
end

u_means= []
u_stds = []
for (uu,gg) in zip(u_storeds,g_storeds)

    push!(u_means, [mean(u, dims=2) for u in uu])
    push!(u_stds, [std(u, dims=2) for u in uu])  
   
end

pp = plot(legend=true)
pp2=plot(legend=true)
for (i,config_key) in enumerate(keys(case_config))
    colors = [:red,:blue,:black]
    u_mean_diff = [norm(u_means[i][idx] - u_true) for idx = 1:length(u_means[i])]
...
savefig(pp, joinpath(fig_save_directory, "mean-over-time.png"))
savefig(pp2, joinpath(fig_save_directory, "dt-over-time.png"))
