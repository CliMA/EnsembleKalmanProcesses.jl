...
for (prior, prior_name, ylim) in [
    ...
    for N_ensemble in N_ensembles
        ...
        for trial in 1:N_trials
            # We now generate the initial ensemble and set up two EKP Inversion objects, one using an accelerator, 
            ...
            # Perform inversion on our two objects
        ...
        if N_ensemble == N_ensembles[end]
            ...
            # error bars (1 standard error away from the mean)
            plot!(0:N_iterations, (mean(all_convs, dims=1)[:] + std(all_convs, dims=1)[:]/sqrt(N_trials)), color = :black, ls= :dash, label = "")
            plot!(0:N_iterations, (mean(all_convs, dims=1)[:] - std(all_convs, dims=1)[:]/sqrt(N_trials)), color = :black, ls= :dash, label = "")
            plot!(0:N_iterations, (mean(all_convs_acc, dims=1)[:] + std(all_convs_acc, dims=1)[:]/sqrt(N_trials)), color = :red, ls= :dash, label = "")
            plot!(0:N_iterations, (mean(all_convs_acc, dims=1)[:] - std(all_convs_acc, dims=1)[:]/sqrt(N_trials)), color = :red, ls= :dash, label = "")
...
savefig(p1, joinpath(fig_save_directory, "exp_sin_multi_comparison_a.png"))
savefig(p2, joinpath(fig_save_directory, "exp_sin_multi_comparison_b.png"))
savefig(p3, joinpath(fig_save_directory, "exp_sin_multi_comparison_c.png"))
