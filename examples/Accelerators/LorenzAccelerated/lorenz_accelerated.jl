...
ekiobj_acc = EKP.EnsembleKalmanProcess(initial_params, truth_sample, truth.obs_noise_cov, Inversion(), accelerator=NesterovAccelerator())
...
for i in 1:N_iter
    ...
    params_i = get_ϕ_final(priors, ekiobj_acc) # the `ϕ` indicates that the `params_i` are in the constrained space    
    g_ens = GModel.run_G_ensemble(params_i, lorenz_settings_G)
    EKP.update_ensemble!(ekiobj_acc, g_ens)
    err_acc[i] = get_error(ekiobj_acc)[end]
    println("Iteration: " * string(i) * ", Error (Nesterov): " * string(err_acc[i]))
end
