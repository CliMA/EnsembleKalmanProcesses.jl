...
if case == "inversion"
    process = Inversion()
    fixed_step = 1.0 # can use DMC for better approximation below
    scheduler = DefaultScheduler(fixed_step)
    #scheduler = DataMisfitController()# terminate in ~6 iterations
    N_iterations = Int(ceil(1000.0/fixed_step))
...
if make_gif
    ...
    gif(anim_sin, "sinusoid_bias_ic_output_parameters_$case.gif", fps = 30) # hide
end
...
savefig(ppp, "sinusoid_bias_ic_output_parameters_$case.png")
