...
# ``f(A, v) = sqrt(A \sin(\phi + t)) + v, \forall t \in [0,2\pi]``, with a random
...
function model(amplitude, vert_shift)
    phi = 2 * pi * rand(rng)
    return (amplitude * sin.(trange .+ phi)).^3 .+ vert_shift
end
nothing # hide
...
if case == "inversion"
    process = Inversion()
    fixed_step = 0.1 # can use DMC for better approximation below
    scheduler = DefaultScheduler(fixed_step)
    #scheduler = DataMisfitController()# terminate in ~6 iterations
    N_iterations = Int(ceil(100.0/fixed_step))

elseif case == "sampler"
    process = Sampler(prior)
    fixed_step = 1e-5 # 1e-4 unstable
    scheduler = DefaultScheduler(fixed_step)
...
initial_u2 = constrained_gaussian("vert_shift", 15, 3, -Inf, Inf)
...
ensemble_kalman_process =
    EKP.EnsembleKalmanProcess(initial_ensemble, y, Γ, process; rng = rng, scheduler = scheduler, accelerator = accelerator, verbose = true)
nothing # hide
...
if make_gif
    ...
    gif(anim_sin, "cube_sinusoid_bias_ic_output_parameters_$case.gif", fps = 30) # hide
end
...
savefig(ppp, "cube_sinusoid_bias_ic_output_parameters_$case.png")
