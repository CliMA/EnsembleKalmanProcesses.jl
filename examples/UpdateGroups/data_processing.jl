...
function process_trajectory_to_data(pw::ProcessingWindow, xn, t)
    ...
    elseif pw.slide_or_batch == "slide"
        ...
        throw(ArgumentError, "ProcessingWindow.slide_or_batch must be \"slide\" or \"batch\", received $pw.slide_or_batch")
    end
    ...
    slow_mean_sw = [mean(vcat(xn[slow_id,indices[sw_start[i]:sw_end[i]]])) for i = 1:n_subwindow]
    fast_mean_sw = [mean(vcat(xn[fast_id,indices[sw_start[i]:sw_end[i]]])) for i = 1:n_subwindow]

    slow_meansq_sw = [mean(vcat(xn[slow_id,indices[sw_start[i]:sw_end[i]]].^2)) for i = 1:n_subwindow]
    fast_meansq_sw = [mean(vcat(xn[fast_id,indices[sw_start[i]:sw_end[i]]].^2)) for i = 1:n_subwindow]

    slowfast_mean_sw = [mean(vcat(
        xn[fast_id,indices[sw_start[i]:sw_end[i]]] .* xn[slow_id,indices[sw_start[i]:sw_end[i]]]
    )) for i = 1:n_subwindow]
