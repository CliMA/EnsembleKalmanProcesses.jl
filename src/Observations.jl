...
#=
function lmul_sqrt_cov!(out, a::SpD, X, idx1, idx2) where {SpD <: SVDplusD}
    tmp = zeros(length(idx1), size(out, 2))
...
function lmul_sqrt_without_build(A, X::AVorM) where {AVorM <: AbstractVecOrMat}
    ...
    for (γs, a) in zip(a_sizes, A)
        idx = (shift[1] + 1):(shift[1] + γs)
        lmul_sqrt_cov!(Y, a, X, idx, idx)
        shift[1] = maximum(idx)
    end
    return Y
end

function lmul_sqrt_cov!(out, a::D, X, global_idx1, local_idx, global_idx2) where {D <: Diagonal}
    view(out, global_idx1, :) .= sqrt.(a.diag[local_idx]) .* X[global_idx2, :]
end

function lmul_sqrt_cov!(out, a::AM, X, global_idx1, local_idx, global_idx2) where {AM <: AbstractMatrix}
    svda = svd(a)
    lmul_sqrt_cov!(out, svda, X, global_idx1, local_idx, global_idx2)
end

function lmul_sqrt_cov!(out, a::SVD, X, global_idx1, local_idx, global_idx2)
    if size(a.U, 1) == size(a.U, 2) # then work with Vt
        view(out, global_idx1, :) .= a.Vt[:, local_idx]' * (sqrt.(a.S) .* a.Vt[:, local_idx]) * X[global_idx2, :]
    else
        view(out, global_idx1, :) .= a.U[local_idx, :] * (sqrt.(a.S) .* a.U[local_idx, :]') * X[global_idx2, :]
    end
end

#=
function lmul_sqrt_cov!(out, a::SpD, X, global_idx1, local_idx, global_idx2) where {SpD <: SVDplusD}
    tmp = zeros(length(global_idx1), size(out, 2))
    for sub_a in get_covs(a)
        lmul_sqrt_cov!(tmp, sub_a, X, 1:size(tmp, 1), local_idx, global_idx2)
        view(out, global_idx1, :) .+= tmp
    end
...
function lmul_sqrt_without_build!(
    ...
    for (block_idx, local_idx, global_idx) in idx_triple
        a = A[block_idx]
        lmul_sqrt_cov!(out, a, X, global_idx, local_idx, global_idx)
    end
end
